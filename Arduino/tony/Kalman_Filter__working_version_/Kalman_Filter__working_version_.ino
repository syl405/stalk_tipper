...
float kalmanCalculate(float newAngle, float newRate,double looptime, float x_angle)
{
...
void loop()
{
  ...
  float newAngle = atan2 (ay,sqrt(ax*ax)+(az*az))*180/pi;
  float gz = dof.calcGyro(dof.gz);
  float gx = dof.calcGyro(dof.gx);
  float newRate = gz;
  x_angle = kalmanCalculate(newAngle,newRate,looptime,x_angle);
